...
public class GenerateBiomes : MonoBehaviour
{
    ...
    public void spawnCityEntity(int posX, int posZ)
    {
        ...
        if (Physics.Raycast(pos, Vector3.down, out hit))
        {
            ...
            if (ground.transform.position.y < waterLevel) Destroy(ground.gameObject);
            Vector3 pos2 = new Vector3(posX + 2, 200, posZ);
            Vector3 pos3 = new Vector3(posX, 200, posZ + 2);

            RaycastHit hit2;

            if (Physics.Raycast(pos2, Vector3.down, out hit2))
            {
                if (Vector3.Distance(hit.point, hit2.point) > 2.12f)
                {
                    Destroy(ground.gameObject);
                    return;
                }
            }
            if (Physics.Raycast(pos3, Vector3.down, out hit2))
            {
                if (Vector3.Distance(hit.point, hit2.point) > 2.12f)
                {
                    Destroy(ground.gameObject);
